...
export class SearchNav extends Component {
  ...
  getCombinedAdvancedSearch = () => {
    const { center, description, keywords, location, photographer, title, yearStart, yearEnd} = this.state;
  ...
  handleSubmit = () => {
    ...
    this.props.fetchImages(this.state.searchQuery, this.state.page, this.getCombinedAdvancedSearch());
  }
  ...
  renderAdvancedSearchLink() {
    return (
      <Typography variant="caption" gutterBottom align="left" className="linkLook">
        <div style={{ paddingTop: 5 }} onClick={ this.toggleAdvancedSearch }>Advanced Search</div>
      </Typography>  
    );
  }
